...
- ~~**多周期共振的去留**~~ **已定案（用户）**：都不选，因为"周期配合度不一样"——看日线要对周线、
  看 15m 要对 1h、做结构可能要跨两级。故**周期对不写死**：`trading_chart` 新增 `compareTo`，
  由模型指定要对比的周期；新增 `requestResonance` 接缝与 `1w` 周期支持。已实盘验证
  `1d vs 1w`（共振）、`15m vs 4h`（背离）、缺省取高一级三种用法。
